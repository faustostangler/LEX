...
Exercises all boundary constraints for 100% mutant elimination.
"""
...
class TestDispositivoNode:
    ...
    def test_tree_construction_and_lookup(self) -> None:
        ...
        assert art.find_node("art_3") == art
...
class TestActAst:
    ...
    def test_act_ast_serialization(self) -> None:
        """Asserts to_dict and from_dict serialization roundtrip and lookup."""
        ...
        found = restored.find_node("art_1")
        assert found is not None
        assert found.label == "Art. 1º"
        assert restored.find_node("art_99") is None
...
class TestNormativeActMutation:
    ...
    def test_valid_mutation_entity(self) -> None:
        ...
        assert mutation.author_act_id == author_id
        assert mutation.confidence_score == 1.0

    def test_alteracao_nr_without_new_text_raises(self) -> None:
        """Asserts that ALTERACAO_NR without new_text raises InvalidMutationPayloadError."""
        target_id = uuid.uuid4()
        author_id = uuid.uuid4()
        with pytest.raises(InvalidMutationPayloadError, match="requires non-empty new_text"):
            NormativeActMutation(
                target_act_id=target_id,
                target_node_path=CanonicalNodePath.from_string("art_3.inc_1"),
                author_act_id=author_id,
    ...
    def test_acrescimo_without_new_text_raises(self) -> None:
        """Asserts that ACRESCIMO without new_text raises InvalidMutationPayloadError."""
        target_id = uuid.uuid4()
        author_id = uuid.uuid4()
        with pytest.raises(InvalidMutationPayloadError, match="requires non-empty new_text"):
            NormativeActMutation(
    ...
    def test_invalid_confidence_score_raises(self) -> None:
        """Asserts that confidence_score outside [0.0, 1.0] raises error."""
        target_id = uuid.uuid4()
        author_id = uuid.uuid4()
