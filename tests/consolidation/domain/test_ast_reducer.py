"""Comprehensive Unit tests for PureAstReducer.

Tests deterministic reduction of base AST trees with chronological mutation streams,
out-of-order sequencing, acréscimos, alterations (NR), revocations, suspensions,
cascading tree effects, and pre-rendered LZ4 HTML/Markdown compilation.
"""
...
class TestPureAstReducer:
    ...
    def test_reduce_alteration_and_revocation(self) -> None:
        ...
        assert "~~II - moralidade e publicidade.~~" in compiled.compiled_markdown
        assert compiled.compiled_version_hash is not None
        assert len(compiled.compiled_version_hash) == 64

    def test_reduce_acrescimo_and_chronological_ordering(self) -> None:
        """Asserts addition of new provisions (ACRESCIMO) and out-of-order mutation sorting."""
        art1 = DispositivoNode(
            node_path=CanonicalNodePath.from_string("art_1"),
            node_type=DispositivoType.ARTIGO,
            label="Art. 1º",
        ...
        base_ast = ActAst(
            act_id=uuid.uuid4(),
            canonical_urn="urn:lex:br:federal:lei:2010;5000",
            title="Lei nº 5.000/2010",
            ementa="Ementa inicial.",
            nodes=[art1],
        )

        author_id = uuid.uuid4()
        # Mutation 2 (Later date): Modifies the added inciso
        m_2020 = NormativeActMutation(
            target_act_id=base_ast.act_id or uuid.uuid4(),
            target_node_path=CanonicalNodePath.from_string("art_1.inc_1"),
            author_act_id=author_id,
            author_dispositivo_ref="Art. 2º",
            mutation_type=MutationType.ALTERACAO_NR,
            new_text="I - redação atualizada em 2020; (NR)",
            publication_date=GazetteDate.from_date(date(2020, 1, 1)),
            effective_date=GazetteDate.from_date(date(2020, 1, 1)),
            extraction_source="lc95_regex",
            confidence_score=1.0,
            mutation_sha256=DocumentHash.from_text("mut-2020"),
        )
        # Mutation 1 (Earlier date): Adds new inciso
        m_2015 = NormativeActMutation(
            target_act_id=base_ast.act_id or uuid.uuid4(),
            target_node_path=CanonicalNodePath.from_string("art_1.inc_1"),
            author_act_id=author_id,
            author_dispositivo_ref="Art. 1º",
            mutation_type=MutationType.ACRESCIMO,
            new_text="I - dispositivo acrescido em 2015;",
            publication_date=GazetteDate.from_date(date(2015, 1, 1)),
            effective_date=GazetteDate.from_date(date(2015, 1, 1)),
            extraction_source="lc95_regex",
            confidence_score=1.0,
            mutation_sha256=DocumentHash.from_text("mut-2015"),
        )

        # Pass in reverse order to verify chronological sorting
        compiled = PureAstReducer.reduce(
            base_ast=base_ast,
            mutations=[m_2020, m_2015],
        )

        assert compiled.total_mutations_applied == 2
        inc1 = compiled.compiled_ast.find_node("art_1.inc_1")
